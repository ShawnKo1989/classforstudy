...
//		Q1214. (고급) 크기가 10인 정수형 배열에 2 이상 100 이하의 정수 10개를 난수 발생하여 채우고
//		이 내용을 출력한 후, 이 배열의 모든 요소들을 ‘가까운 3의 배수’로 만든 배열의 내용을 출력해보세요.
//		(실행결과)
...
//		3의 배수로 조정된 배열 : [12, 96, 69, 75, 66, 99, 12, 81, 96, 57]
		...
		System.out.print("생성된 배열 : "+ Arrays.toString(arr));
		System.out.println();
		for(int i=0;i<=arr.length-1;i++) {
			if(arr[i]%3==2) {
				arr[i] ++;
		...
		System.out.print("3의 배수로 조정된 배열 : " + Arrays.toString(arr));
